...
ABSL_FLAG(std::string, config, "./config/proxy.yaml", "config file");
ABSL_FLAG(int, service_port, 70050, "node service port");  // deprecated,

using LinkFactory = primihub::network::LinkFactory;                                                        // remove in future
/**
 * @brief
 *  Start Apache arrow flight server with NodeService and DatasetService.
 */

void RunServer(primihub::VMNodeProxyInterface* node_service,
               primihub::DataServiceProxyImpl* dataset_service,
               int service_port) {
    // Initialize server
...
int main(int argc, char **argv) {
    ...
    signal(SIGINT, [](int sig) {
        LOG(INFO) << "Node proxy received SIGINT signal, shutting down...";
        exit(0);
    });
    ...
    auto ret = svr_cfg.initServerConfig(config_file);
    if (ret != primihub::retcode::SUCCESS) {
        LOG(ERROR) << "init server config failed";
        return -1;
    }
    auto& host_config = svr_cfg.getServiceConfig();
    ...
    auto& delegate_svr = svr_cfg.DelegateServer();
    auto link_mode = LinkFactory::GetLinkMode(delegate_svr.mode);
    auto link_ctx = LinkFactory::createLinkContext(link_mode);
    // service for dataset register
    auto data_service = std::make_unique<primihub::DataServiceProxyImpl>(
        link_ctx.get(), delegate_svr.host_info);
    auto node_service = std::make_unique<primihub::VMNodeProxyInterface>(
        link_ctx.get(), delegate_svr.host_info);
